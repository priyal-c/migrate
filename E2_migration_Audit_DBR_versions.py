...
newjobslist=client.get_list_jobs() ## fetch list all the jobs

# COMMAND ----------

clusterslist=client.get_cluster_list() #fetch list of all the interactive cluster
...
def get_DBR_version(version):
  ...
  for i in range(len(DBR_versions_list)):
    if DBR_versions_list[i]["key"]==version:
        return (DBR_versions_list[i]["name"]) #=clusterslist[0]["spark_version"])
  return("***DBR Runtime version not avaiable***") 

# COMMAND ----------

#print DBR version of Interactive as well as job cluster
print("ClusterName/JobName | DBR Version\n")
for i in range(len(newjobslist)):
  if "new_cluster" in newjobslist[i]["settings"]:
    print(("%s | %s"%(newjobslist[i]["settings"]["name"],get_DBR_version(newjobslist[i]["settings"]["new_cluster"]["spark_version"]))))
for i in (range(len(clusterslist))):
  print("%s | %s" %(clusterslist[i]["cluster_name"],get_DBR_version(clusterslist[i]["spark_version"])))
