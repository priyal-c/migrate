...
# MAGIC %md
# MAGIC ###Script to Map Old Job IDs to New Job Ids using Rest APIs starts here

# COMMAND ----------

#client pointing to current E2 workspace
...
newjobslist=client.get_list_jobs() ##list all the jobs in E2 shard

# COMMAND ----------

# client pointing to old legacy workspace

url_old = "" #provide Host URL of legacy workpspace prefix with https://
token_old = "" # provide PAT token of legacy workspace

client_old = migrateclient(token_old, url_old)

# COMMAND ----------

masterlegcyjobslist=client_old.get_list_jobs() ##list all the jobs on legacy shard



# COMMAND ----------

# compare the newjob list and with legacy job list to get job ids with same name, then print the list
print ("Job_name | Old_job_id | New_job_id" )
for i in range(len(newjobslist)):
    #print(newjobslistlist[i]["settings"]["name"] + "new")
    for j in range(len(masterlegcyjobslist)):
           #print(masterlegcyjobslist[j]["settings"]["name"])
           if newjobslist[i]["settings"]["name"]== masterlegcyjobslist[j]["settings"]["name"]:
                   print ("%s | %i | %i" %(masterlegcyjobslist[j]["settings"]["name"],masterlegcyjobslist[j]["job_id"],newjobslist[i]["job_id"]))
